// Worker Bundle - Built 2026-09-24T07:07:25Z
// Version: 0.0.0
...
// src/middleware/errorHandler.js
function errorHandler2(error3, c) {
  console.error("Error in request handler:", error3);
  if (error3 instanceof APIError) {
    return c.json(error3.toJSON(), error3.statusCode);
  }
  if (error3.name === "ValidationError" || error3.status === 400) {
    return c.json({
      error: error3.message || "Validation error",
      status: 400
    }, 400);
  }
  if (error3.name === "AuthenticationError" || error3.status === 401) {
    return c.json({
      error: error3.message || "Authentication required",
      status: 401
    }, 401);
  }
  if (error3.name === "NotFoundError" || error3.status === 404) {
    return c.json({
      error: error3.message || "Resource not found",
      status: 404
    }, 404);
  }
  return c.json({
...
function asyncHandler(fn2) {
  return async (c, next) => {
    try {
      return await fn2(c, next);
    } catch (error3) {
      return errorHandler2(error3, c);
    }
  };
}
...
// src/middleware/productLimit.js
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
...
var StoreSettingsService = class {
  static {
    __name(this, "StoreSettingsService");
  }
...
// src/middleware/productLimit.js
async function productLimitMiddleware(c, next) {
  ...
  try {
    ...
    if ((productLimit === null || productLimit === void 0 || productLimit === "") && c.env.PRODUCT_LIMIT) {
      productLimit = c.env.PRODUCT_LIMIT;
    }
    if (productLimit === null || productLimit === void 0 || productLimit === "") {
      return next();
    }
    ...
    console.error("Product limit middleware error:", error3);
    return next();
  }
}
__name(productLimitMiddleware, "productLimitMiddleware");
...
router2.get("/:id", asyncHandler(async (c) => {
  const kvNamespace = getKVNamespace(c.env);
  const collectionService = new CollectionService(kvNamespace);
  const collection = await collectionService.getCollection(c.req.param("id"));
  return c.json(collection);
}));
router2.get("/:id/products", asyncHandler(async (c) => {
  const kvNamespace = getKVNamespace(c.env);
  const collectionService = new CollectionService(kvNamespace);
...
var storefront_default = router3;
...
router4.post("/create-cart-checkout-session", asyncHandler(async (c) => {
...
router4.get("/checkout-session/:sessionId", asyncHandler(async (c) => {
...
// src/routes/public/imageProxy.js
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
...
var imageProxy_default = router5;

// src/routes/public/storeSettings.js
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
...
var storeSettings_default = router6;
var contactEmailRouter = new Hono2();
...
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
var ProductStripeService = class {
  static {
    __name(this, "ProductStripeService");
  }
...
__name(isValidObject, "isValidObject");
...
router15.get("/store-settings", asyncHandler(async (c) => {
  ...
  const settingsService = new StoreSettingsService(kvNamespace);
  const settings = await settingsService.getSettings();
  if ((settings.productLimit === null || settings.productLimit === void 0 || settings.productLimit === "") && c.env.PRODUCT_LIMIT) {
...
__name(persistGeneratedImage, "persistGeneratedImage");
...
__name(generateFromOpenRouter, "generateFromOpenRouter");
...
router16.post("/generate-and-store", asyncHandler(async (c) => {
  const body = await c.req.json();
  const prompt = typeof body?.prompt === "string" ? body.prompt.trim() : "";
  if (!prompt) {
    throw new ValidationError("prompt is required");
  }
  const filename = typeof body?.filename === "string" && body.filename.trim() ? body.filename.trim() : "generated.png";
...
// src/routes/admin/agent.js
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();

// src/routes/admin/agentTools.js
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
var MAX_TOOL_ITERATIONS = 12;
var FONT_IDS = FONT_OPTIONS.map((font) => font.id);
...
var AGENT_WRITABLE_DEV_KEYS = new Set(
...
function buildSystemPrompt({ hasReferences = false } = {}) {
  return [
    ...
    hasReferences ? "The user attached reference images. They are passed to generate_product_image automatically; you do not need to describe or upload them." : "The user attached no reference images, so describe the model, garment and artwork in words if generating merch.",
...
function trimProduct(p) {
  ...
  return {
    ...
    imageUrl: p.imageUrl,
    images: Array.isArray(p.images) ? p.images.slice(0, 5) : void 0,
    ...
    variants: Array.isArray(p.variants) ? p.variants.map((v) => ({ id: v.id, name: v.name, price: v.price, hasCustomPrice: v.hasCustomPrice })) : void 0,
    variants2: Array.isArray(p.variants2) ? p.variants2.map((v) => ({ id: v.id, name: v.name, price: v.price, hasCustomPrice: v.hasCustomPrice })) : void 0,
    archived: !!p.archived
  };
}
__name(trimProduct, "trimProduct");
...
function productPayload(a, { creating = false } = {}) {
  ...
  if (a.price !== void 0) payload.price = Number(a.price);
  if (creating && payload.currency === void 0) payload.currency = "USD";
  if (creating && payload.description === void 0) payload.description = "";
...
__name(productPayload, "productPayload");
...
__name(summarizeResult, "summarizeResult");
async function executePageTool(dispatch, name, args) {
  const a = args || {};
  const slug = String(a.slug || "").trim();
  if (name === "get_page") {
    const r2 = await dispatch(`/api/admin/storefront/pages/${encodeURIComponent(slug)}`);
    if (!r2.ok) return { status: r2.status, data: r2.body };
    const content = Array.isArray(r2.body?.data?.content) ? r2.body.data.content : [];
    return {
      status: r2.status,
      data: {
        slug: r2.body.slug,
        updatedAt: r2.body.updatedAt,
        rootProps: r2.body?.data?.root?.props ?? {},
        content
      }
    };
  }
  if (name === "create_page") {
    const r2 = await dispatch("/api/admin/storefront/pages", {
      method: "POST",
      body: JSON.stringify({ slug })
    });
    if (!r2.ok && !String(r2.body?.error || "").includes("already exists")) {
      return { status: r2.status, data: r2.body };
    }
    return { status: 200, data: { slug, created: true } };
  }
  if (name === "delete_page") {
    const r2 = await dispatch(`/api/admin/storefront/pages/${encodeURIComponent(slug)}`, { method: "DELETE" });
    return { status: r2.status, data: r2.ok ? { slug, deleted: true } : r2.body };
  }
  ...
  const nextData = {
    ...
    root: {
      props: {
        ...
        ...a.seoTitle !== void 0 ? { title: a.seoTitle } : {},
        ...a.seoDescription !== void 0 ? { description: a.seoDescription } : {}
      }
    }
  };
...
__name(executePageTool, "executePageTool");
...
__name(executeThemeUpdate, "executeThemeUpdate");
async function executeAgentTool(c, name, args, dispatch) {
  const a = args || {};
  switch (name) {
    case "list_products": {
...
var TOOL_NAMES = TOOL_DEFINITIONS.map((tool) => tool.function.name);

// src/routes/admin/agent.js
var router17 = new Hono2();
var appRef = null;
function setAgentApp(app2) {
  appRef = app2;
}
...
__name(dispatchFrom, "dispatchFrom");
async function openRouterBase(env2) {
  const override = env2?.OPENROUTER_BASE;
  if (typeof override === "string" && override.trim()) {
    return override.trim().replace(/\/$/, "");
  }
  return OPENROUTER_BASE;
}
__name(openRouterBase, "openRouterBase");
async function callOpenRouter(apiKey, model, messages, env2) {
  const res = await fetch(`${await openRouterBase(env2)}/chat/completions`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${apiKey}`,
      "Content-Type": "application/json"
...
router17.post("/chat", asyncHandler(async (c) => {
  ...
  const actions = [];
  const dispatch = /* @__PURE__ */ __name((path, init) => dispatchFrom(c, path, init), "dispatch");
  for (let i = 0; i < MAX_TOOL_ITERATIONS; i++) {
    const completion2 = await callOpenRouter(apiKey, selectedModel, chatMessages, c.env);
    const choice = completion2?.choices?.[0]?.message;
    if (!choice) {
    ...
    for (const call of toolCalls) {
      ...
      try {
        ...
        chatMessages.push({
          ...
          content: JSON.stringify(result.data ?? { status: result.status }).slice(0, 24e3)
        });
      } catch (err) {
        action.ok = false;
...
function registerRoutes(app2) {
  ...
  app2.route("/api/storefront", storefront_default);
  app2.route("/api", checkout_default);
